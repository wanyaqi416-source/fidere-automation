...
| DA-002 | 新增 -> 安全验证 -> Admin审批 -> Client可见 | In Progress；代码已接入，未真实执行 | `npm run test:digital-address` |
...
| 钱包地址 | 必填，最多200字符；真实Run必须提供受控Sandbox地址，不生成或猜测收款钱包 |
...
不持久化钱包原文、邮箱、密钥或认证数据。动作attempt在点击前写入，报告次数按尝试边界统计。
...
首次实际闭环仍需提供受控钱包地址/网络并授权一个命名Run，未验证前不标记DA-002 Ready。
