...
读取原费率 -> 单项保存/回读 -> 默认Client试算 -> 确认/安全验证各一次 -> 唯一原TRF/TXN。
资金互转：自动完成 -> Admin同TXN金额/费用/实际到账只读核对。
用户转账：Admin唯一候选及详情核对 -> 批准一次 -> 已批准即业务结束，无余额/流水尾检查。
每项恢复原类型和值。`ATFM6-20260910-01` 六笔USD真实业务均完成，含原订单核查与Resume，未补交。
能力Ready，默认回归排除；详见[六笔矩阵与历史报告](./transfer-fee-matrix.md)。HKD/SGD示例因余额不足未实际执行。

### 固定互转手续费 / ATF-001、ATF-002

Admin 业务配置 -> 账户类型配置 -> 巴林 USD 固定互转费
-> 保存后回读 -> 默认 Client 巴林到香港 USD 两金额试算
-> 授权金额单次提交和安全验证 -> 唯一原 TRF 完成及金额核对 -> 恢复原配置。

ATF-001 只编辑取消和验证原费，真实 Dry Run 通过后 Ready；ATF-002 真实执行加原订单只读复核通过，Ready / Real E2E Verified=Yes。
`ATF002-20260910-01` 原互转已完成：11.13 USD、费用0.37、Client确认页与Admin同TXN实际到账均10.76；原40 USD费已恢复，禁止重跑。
首次失败源于误把API的actualAmount当作净额，不是产品金额错误；原失败报告保留。独立ATF-002-RECON只读验证原订单、Admin真实金额标签与配置恢复，不提交或审批。
新下拉框支持免手续费/固定/百分比：ATF-003/004/005真实表单Dry Run均Ready（3/3），不代表三种收费的真实转账已执行。ATF-001按当前类型只读试算；配置恢复包含原类型和值，检测并发更改时禁止覆盖。详见 [用例说明](./account-transfer-fee.md)。
